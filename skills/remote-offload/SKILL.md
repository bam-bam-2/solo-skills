---
name: "macmini-offload"
description: "무거운 작업(스크래핑, 미디어 인코딩, 빌드/설치, 대량 API 수집, 긴 문서 읽기, 대용량 파일 처리)을 맥미니로 넘길 때 사용. Aside가 도는 맥북에어는 8GB라 상시 메모리 부족이므로, CPU/RAM/시간을 많이 쓰는 일은 기본적으로 맥미니에서 돌린다."
---

> **이 스킬이 시스템에 하는 일 (설치 전 확인)**
>
> - `~/.ssh/config`에 등록된 원격 호스트로 **SSH 접속해 명령을 실행**합니다.
> - 원격이 응답하지 않으면 **로컬로 대체 실행하지 않고 중단**합니다. 의도된 동작입니다.
...
사용자의 탭이 죽는다. **무거운 건 맥미니로 넘기는 게 기본값이다.**

## 접속

```bash
ssh mac-mini          # Tailscale (100.75.197.6) — 기본으로 이걸 쓴다
ssh mac-mini-lan      # 같은 LAN일 때 (mac-mini.local)
```

비대화형에서는 `-o BatchMode=yes -o ConnectTimeout=10`을 붙인다.
PATH가 필요한 명령(brew 설치물 등)은 `ssh mac-mini 'bash -lc "..."'`로 로그인 셸을 태운다.

## 작업공간 규약

```
~/Projects/ops/offload/
├── run.sh          러너 (아래 참조)
...
... 스크립트 ...
EOF
scp -q "$TMP/job.js" mac-mini:~/Projects/ops/offload/jobs/job.js
ssh mac-mini 'cd ~/Projects/ops/offload && node jobs/job.js'
```
...
```bash
R=~/Projects/ops/offload/run.sh
ssh mac-mini "$R start 'node jobs/scrape.js' scrape"   # -> JOB_ID=scrape-20260811-124757
ssh mac-mini "$R status scrape-20260811-124757"        # running (pid=...) 또는 ### EXIT=0
ssh mac-mini "$R tail   scrape-20260811-124757"        # 마지막 40줄
ssh mac-mini "$R log    scrape-20260811-124757"        # 전체 로그
ssh mac-mini "$R list"                                 # 최근 20개 작업
```
...
```bash
scp mac-mini:~/Projects/ops/offload/out/result.csv <세션 artifacts 디렉토리>/
scp -r mac-mini:~/Projects/ops/offload/out/frames  <세션 tmp 디렉토리>/
```
...
```js
const { chromium } = require('playwright');   // ~/Projects/ops/offload 에서 실행
(async () => {
...
`ffmpeg`는 `/opt/homebrew/bin/ffmpeg`. 인코딩·GIF 변환은 전부 맥미니에서 한다.
...
```bash
ssh mac-mini 'bash -lc "cat ~/Projects/ops/offload/jobs/doc.txt | claude -p --model claude-sonnet-4-5"'
```
...
없는 게 필요하면 `ssh mac-mini 'bash -lc "brew install <pkg>"'`로 설치하고 이 목록을 갱신한다.

## 주의

- 맥미니는 이미 launchd 상주 작업이 여러 개 돈다 (`atlas-server`, `atlas-kakao-poll`,
...
- 자격증명은 맥미니에 있는 기존 `.env`에서 런타임에만 읽고, 값을 출력하지 않는다.
