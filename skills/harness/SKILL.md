---
name: harness
description: "하네스를 구성합니다. 전문 에이전트를 정의하며, 해당 에이전트가 사용할 스킬을 생성하는 메타 스킬. (1) '하네스 구성해줘', '하네스 구축해줘' 요청 시, (2) '하네스 설계', '하네스 엔지니어링' 요청 시, (3) 새로운 도메인/프로젝트에 대한 하네스 기반 자동화 체계를 구축할 때, (4) 하네스 구성을 재구성하거나 확장할 때, (5) '하네스 점검', '하네스 감사', '하네스 현황', '에이전트/스킬 동기화' 등 기존 하네스 운영/유지보수 요청 시 사용. 단, ~/Projects/meta/ 스코프의 하네스·스킬·설정 작업은 meta-orchestrator 스킬이 우선."
---
...
#### 5-6. 알토스(Atlas) 등록 — 필수, 건너뛰기 금지

하네스 구성이 워크플로우나 launchd 자동화를 포함하면 **반드시** 알토스 대시보드(`~/Projects/atlas/`)에 등록한다. 등록 3종 세트:

1. **도식**: `~/Projects/atlas/flows/{slug}.mmd` — 헤더 4줄(title/project/code/trigger) + `click X call atlasInfo("...")` 주석. 검증: 헤더 떼고 `npx -y @mermaid-js/mermaid-cli -i <파일> -o /tmp/check.svg` (mermaid 10 함정은 알토스 CLAUDE.md 참조)
2. **자동화 한글 라벨**: 새 launchd 잡마다 `~/Projects/atlas/scan.py`의 `AUTOMATION_LABELS`에 `"라벨": ("한글이름","설명","카테고리")` 추가
3. **상태 배지**: `scan.py`의 `FLOW_LAUNCHD`에 `'slug': 'launchd라벨'` 매핑 (+ `skill_categories.py` 카테고리 배치)

등록 후 `python3 ~/Projects/atlas/scan.py` 1회 실행으로 즉시 반영 확인. 스킬 카드는 자동 스캔이라 등록 불필요.
**백스톱**: scan.py에 미등록 감지기가 하드코딩되어 있어(2026-07-07) 등록을 빼먹은 launchd 잡은 대시보드에 "⚠️ 미등록 자동화" 경고로 뜨고 scan.log에 기록된다 — 경고가 보이면 이 섹션대로 등록하라.
...
- [ ] **알토스 등록 3종 세트** (Phase 5-6) — 워크플로우/launchd 포함 하네스면 필수: flows/*.mmd 도식 + AUTOMATION_LABELS + FLOW_LAUNCHD, `python3 scan.py`로 반영 확인
