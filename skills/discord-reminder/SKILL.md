---
name: "macmini-discord-reminder"
description: "Use when the user asks for a one-time or recurring reminder that should run on the mac-mini and arrive by Discord DM."
---
...
Use the mac-mini instead of Aside routine slots for scheduled reminders.
...
2. Store the executable script at `~/Projects/reminders/<slug>.sh` on `mac-mini` with mode `700`.
3. Load Discord credentials only at runtime from `~/Projects/getback/discord_bot/.env`. Never print or copy token values.
...
5. Store the LaunchAgent at `~/Library/LaunchAgents/com.bambam.reminder-<slug>.plist`.
   ...
   - **Never use `StartInterval` on this mac-mini.** Verified 2026-08-16: every
...
6. Write stdout and stderr to `~/Projects/reminders/<slug>.log`.
