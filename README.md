...
각 스킬은 **에이전트가 읽고 수행하는 작업 절차서**이고, 그중 일부에는 제가 실제로 돌리는 **실행 스크립트 9개**가 같이 들어 있습니다.
...
**humanize-korean** — AI가 쓴 한글을 사람 글로 되돌립니다.
`~가 아니라` 대조 구문, `여정`, `굴러가다` 같은 AI 특유의 표현을 잡아냅니다. 제가 계속 지적받으면서 쌓은 금지어 목록이 그대로 들어 있습니다.

**voice-dna-creator** — 글 샘플을 넣으면 그 사람의 문체를 수치로 뽑아냅니다.
길이·줄바꿈·어미·인칭 분포를 재서, 다음 글을 그 범위 안에서 쓰게 합니다. 감으로 쓰다가 "네 문체가 아니다"라는 말을 듣고 만들었습니다.

**style-skill-creator** — 특정 문체를 학습시키는 스킬을 만드는 스킬입니다.

### 회의·행사

**meeting-summary** — 긴 회의록에서 결정사항과 할 일만 남깁니다.
요약이 아니라 **추출**입니다. 안건별 논의 과정은 버리고 "누가 언제까지 뭘"만 남깁니다.

**workshop-prep** — 워크숍 한 건을 준비하는 전체 절차입니다. 커리큘럼 설계, 준비물, 당일 진행표까지.

**event-sales-script** — 오프라인 행사를 단톡방과 SNS에서 팔기 위한 멘트 세트를 만듭니다.
같은 행사라도 채널마다 톤이 달라야 해서, 채널별로 따로 뽑습니다.

**community-launch** — 커뮤니티 기수 하나를 여는 절차입니다. 모집부터 온보딩까지.

### 에이전트 운영

**orchestration** — 여러 에이전트에게 일을 나눠 맡기고 결과를 합치는 절차입니다.

**harness** — 전문 에이전트를 정의하고, 그 에이전트가 쓸 스킬까지 만들어내는 메타 스킬입니다.

**computer-use** — 화면을 직접 보고 조작해야 할 때 쓰는 절차입니다.

**notion-delete** — 노션 페이지를 안전하게 아카이브합니다.
